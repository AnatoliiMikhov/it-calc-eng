...
exports.handler = async (event, context) => {
    ...
    if (event.httpMethod !== 'GET') {
        return {
            statusCode: 405,
            body: JSON.stringify({ message: 'Method Not Allowed', error: 'Only GET requests are supported.' }),
            headers: { 'Allow': 'GET' }
        };
    }
    ...
    try {
        ...
        if (!doc.exists) {
            ...
            return {
                statusCode: 404,
                body: JSON.stringify({ message: 'Not Found', error: 'The requested rates configuration was not found.' })
            };
        }
