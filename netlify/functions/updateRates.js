...
exports.handler = async (event, context) => {
    ...
    if (event.httpMethod !== 'POST') {
        return {
            statusCode: 405,
            body: JSON.stringify({ message: 'Method Not Allowed', error: 'Only POST requests are supported for this endpoint.' }),
    ...
    if (!user) {
        ...
        return {
            statusCode: 401,
            body: JSON.stringify({ message: 'Unauthorized', error: 'You must be logged in to update rates.' }),
            headers: { 'Content-Type': 'application/json' },
        };
    }
    ...
    if (!roles.includes('admin')) {
        ...
        return {
            statusCode: 403,
            body: JSON.stringify({ message: 'Forbidden', error: 'You do not have permission to update rates. Admin access required.' }),
            headers: { 'Content-Type': 'application/json' },
        };
    }
    ...
    try {
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({ message: 'Failed to update rates', error: error.message }),
            headers: { 'Content-Type': 'application/json' },
        };
    }
};